#!/usr/bin/env bash
set -euo pipefail

STREAM=/home/casbot/stream_ros2.py
MOTIONS=/home/casbot/motions

run_take()
{
    local take_number=$1
    local speaker=$2
    local variant=$3
    local trajectory=$4
    local video_speedup=$5

    echo
    echo "============================================================"
    echo "TAKE ${take_number}: ${speaker} / ${variant}"
    echo "Raw video: ${speaker}_${variant}_raw.mp4"
    echo "Post-production gesture speedup: ${video_speedup}x"
    echo "Start a fresh camera recording before typing 'go'."
    echo "============================================================"

    python3 "$STREAM" \
        --traj "$trajectory" \
        --enable-feedback \
        --arms both \
        --split-arm-topics \
        --speed 0.25 \
        --command-rate 50 \
        --ramp 20 \
        --movej-first \
        --movej-vel 0.01 \
        --track-tol 0.15

    echo "CLEAN TAKE: ${speaker} / ${variant}"
}

run_take 01 neerja_forward presentation \
    "$MOTIONS/02_neerja_forward/presentation.npz" \
    3.526160225500
run_take 02 neerja_forward casual \
    "$MOTIONS/02_neerja_forward/casual.npz" \
    3.433737118416

run_take 03 sonia presentation \
    "$MOTIONS/03_sonia/presentation.npz" \
    3.588223572764
run_take 04 sonia casual \
    "$MOTIONS/03_sonia/casual.npz" \
    3.995580209516

run_take 05 ava presentation \
    "$MOTIONS/04_ava/presentation.npz" \
    3.998302736836
run_take 06 ava casual \
    "$MOTIONS/04_ava/casual.npz" \
    3.968433393352

run_take 07 neerja_expressive presentation \
    "$MOTIONS/05_neerja_expressive/presentation.npz" \
    3.855373630388
run_take 08 neerja_expressive casual \
    "$MOTIONS/05_neerja_expressive/casual.npz" \
    3.539371902648

echo
echo "ALL EIGHT NO-TABLE SPEAKER TAKES FINISHED CLEANLY."
echo "Move the table into place, then run /home/casbot/record_tabletop.sh."
